      ...
      <div class="flex" style="height: calc(100vh - 4rem)">
          ...
          <div class="flex-1 overflow-y-auto">
            ...
            @for (c of contacts(); track c.id) {
              <button
                ...
                [class]="
                  contactSelectionne()?.id === c.id ? contactActifClass : contactInactifClass
                "
              >
                <div class="flex items-center gap-3 w-full">
                  ...
                  @if (c.nonLus > 0) {
                    <span
                      class="shrink-0 bg-primary-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold"
                    >
                      {{ c.nonLus }}
                    </span>
                  }
        ...
        <div class="flex-1 flex flex-col min-w-0">
          @if (!contactSelectionne()) {
            ...
            <!-- En-tete -->
            <div
              class="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3 shrink-0"
            >
              <div
              ...
              <div>
                <p class="font-semibold text-gray-900 text-sm">
                  {{ contactSelectionne()!.nomComplet }}
                </p>
                <p class="text-xs text-gray-500">
                  {{ contactSelectionne()!.nomEntreprise ?? contactSelectionne()!.role }}
                </p>
              </div>
            </div>
              ...
              @for (msg of messagesAffiches; track msg.id) {
                <div [class]="getMsgRowClass(msg)">
                  @if (msg.expediteurId !== auth.user()?.userId) {
                    <div
                      class="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 self-end"
                      [class]="getRoleColor(contactSelectionne()!.role)"
                    >
...
export class MessagerieComponent implements OnInit, OnDestroy, AfterViewChecked {
    ...
    this.sub = this.svc.messageRecu$.subscribe((msg) => {
      ...
      if (contact && msg.expediteurId === contact.id) {
        ...
        this.contacts.update((list) =>
          list.map((c) => (c.id === msg.expediteurId ? { ...c, nonLus: c.nonLus + 1 } : c)),
        );
        this.cdr.detectChanges();
      }
    });
  ...
  selectionnerContact(contact: Contact) {
    ...
    // Informer le service : cette conversation est active
