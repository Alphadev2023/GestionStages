...
export function MessageriePage() {
  ...
  // Sélecteurs individuels — chacun ne re-rend le composant que si SA valeur change
  const messages = useMessagerieStore((s) => s.messages);
  const contactActif = useMessagerieStore((s) => s.contactActif);
  const nonLus = useMessagerieStore((s) => s.nonLus);

  // Actions — références stables entre les renders, safe pour les deps
  const setContacts = useMessagerieStore((s) => s.setContacts);
  const setContactActif = useMessagerieStore((s) => s.setContactActif);
  const setMessages = useMessagerieStore((s) => s.setMessages);
  const resetNonLus = useMessagerieStore((s) => s.resetNonLus);
  ...
  }, [contactActif?.id, resetNonLus, setMessages]);
  ...
  async function envoyer() {
    const dest = contactActif;
    if (!contenu.trim() || !dest || envoi) return;
    setEnvoi(true);
    const texte = contenu;
    setContenu("");
    try {
      const res = await messagerieService.envoyer({
            ...
            <div className="flex-1 overflow-y-auto">
              {!contacts ? (
                ...
                (contacts ?? []).map((c) => (
                  <button
                    key={c.id}
                    onClick={() => selectionnerContact(c)}
                    className={
                      "w-full px-4 py-3 border-l-4 transition-colors text-left hover:bg-gray-50 " +
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={
                          ...
                          (ROLE_COLORS[c.role] ?? "bg-gray-500")
                        }
                      >
                ...
                <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3 shrink-0">
                  <div
                    className={
                      ...
                      (ROLE_COLORS[contactActif.role] ?? "bg-gray-500")
                    }
                  >
                    {contactActif.nomComplet.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p className="font-semibold text-gray-900 text-sm">
                      {contactActif.nomComplet}
                    </p>
                    <p className="text-xs text-gray-500">
                      {contactActif.nomEntreprise ?? contactActif.role}
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  {messages.length === 0 && (
                    <p className="text-center text-gray-400 text-sm py-8">
                      Aucun message â€” commencez la conversation
                    </p>
                  )}
                </div>
