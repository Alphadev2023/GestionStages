...
export function CandidaturesPage() {
  ...
  function confirmerTraitement() {
    ...
    const id = toast.loading(action==='ACCEPTEE'?'Acceptation...':'Refus...');
    traiter.mutate({ id: selected.id, statut: action, feedback: feedback||undefined }, {
      onSuccess: () => {
        toast.dismiss(id);
        toast.success(action==='ACCEPTEE'
          ? 'Candidature acceptee — convention creee automatiquement !'
          : 'Candidature refusee');
        setSelected(null); setFeedback(''); setAction(null);
      },
  ...
  return (
    <>
      ...
      <div className="ml-64 min-h-screen bg-gray-50">
        ...
        <main className="p-6">
          ...
          <Card className="mb-6">
            ...
            <select value={offreId??''} onChange={e=>setOffreId(Number(e.target.value)||null)}
            ...
            {!offreId && <p className="text-xs text-gray-400 mt-2">Selectionnez une offre pour voir les candidatures recues</p>}
          </Card>
                        ...
                        {c.statut==='EN_ATTENTE' && (
                          <>
                            <Button size="sm" variant="success"
                              onClick={()=>{ setSelected(c); setAction('ACCEPTEE'); }}>
                              Accepter
                            </Button>
                            <Button size="sm" variant="danger"
                              onClick={()=>{ setSelected(c); setAction('REFUSEE'); }}>
                              Refuser
                            </Button>
                          </>
                        )}
      ...
      <Modal
        ...
        onClose={()=>{ setSelected(null); setAction(null); setFeedback(''); }}
        title={action==='ACCEPTEE' ? 'Accepter cette candidature' : 'Refuser cette candidature'}
        size="sm"
      >
        <div className="space-y-4">
          {selected && (
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                <p className="font-medium text-gray-900">{selected.nomEtudiant}</p>
          ...
          {action==='ACCEPTEE' && (
            <div className="p-3 bg-success-50 rounded-lg border border-success-200">
              <p className="text-sm text-success-700 font-medium">
                En acceptant, une convention sera creee automatiquement et assignee a un enseignant.
              </p>
            </div>
          )}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              {action==='REFUSEE' && <span className="text-danger-600 ml-1">*</span>}
            </label>
            <textarea value={feedback} onChange={e=>setFeedback(e.target.value)} rows={3}
              className="input-field resize-none"
              placeholder={action==='ACCEPTEE'
            ...
            {action==='REFUSEE' && !feedback && (
              <p className="text-xs text-gray-400 mt-1">Expliquez les raisons du refus pour aider l etudiant</p>
            )}
          </div>

          <div className="flex gap-3 pt-2">
            <Button variant="secondary" className="flex-1"
              onClick={()=>{ setSelected(null); setAction(null); setFeedback(''); }}>
              Annuler
            </Button>
            <Button
              ...
              {traiter.isPending ? 'Traitement...' : action==='ACCEPTEE' ? 'Confirmer l acceptation' : 'Confirmer le refus'}
            </Button>
          </div>
        </div>
      </Modal>
