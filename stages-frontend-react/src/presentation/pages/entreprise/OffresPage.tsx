...
const schema = Yup.object({
  ...
  dureeMois: Yup.number()
    .min(1, "Minimum 1 mois")
    .required("Duree obligatoire"),
  dateDebut: Yup.string()
    .required("Date de debut obligatoire")
    .test(
      "future",
      "La date de debut doit etre aujourd hui ou dans le futur",
      (v) => !v || new Date(v) >= new Date(today),
    ),
  dateExpiration: Yup.string()
    .required("Date d expiration obligatoire")
    .test(
      "after-debut",
      "La date d expiration doit etre apres la date de debut",
      function (v) {
    ...
    .test("match-duree", "La duree ne correspond pas aux dates", function (v) {
      ...
      const diff =
        (new Date(v).getTime() - new Date(dateDebut).getTime()) /
        (1000 * 60 * 60 * 24 * 30.44);
    ...
    <>
      ...
      <div className="ml-64 min-h-screen bg-gray-50">
        ...
        <main className="p-6">
          ...
          {isLoading ? (
            <Spinner text="Chargement des offres..." />
          ) : (
            <>
              {/* Grille */}
                  ...
                  >
                    <div className="p-4 border-b border-gray-100">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <Badge
                          variant={o.statut === "ACTIVE" ? "accepte" : "refuse"}
                    ...
                    <div className="p-4 flex-1 space-y-1.5 text-xs text-gray-600">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{o.localisation}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Duree</span>
                        <span className="font-medium">{o.dureeMois} mois</span>
                      </div>
                      {o.remuneration && (
                        <div className="flex justify-between">
                          <span className="text-gray-400">Remuneration</span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{o.nombrePostes}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Expire</span>
                        <span className="font-medium">
                          {o.dateExpiration.slice(0, 10)}
                        </span>
                      </div>
                      {detailId === o.id && (
                        <div className="mt-3 pt-3 border-t border-gray-100 overflow-hidden">
                          <p className="text-xs text-gray-700 mb-2 break-words overflow-hidden">
                            {o.description}
                          </p>
                          {o.competencesRequises && (
                            <div className="flex flex-wrap gap-1">
                              {o.competencesRequises.split(",").map((c, i) => (
                                <span
                                  key={i}
                                  className="text-xs bg-primary-50 text-primary-700 px-1.5 py-0.5 rounded border border-primary-100"
                                >
                                  {c.trim()}
                                </span>
                              ))}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-6 px-1">
                  <p className="text-sm text-gray-500">
                    Page {page + 1} sur {totalPages} — {offresTriees.length}{" "}
                    offres
                  </p>
      ...
      <Modal
        ...
        title="Publier une offre de stage"
        size="lg"
      >
        <Formik
            ...
            <Form className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Titre du poste <span className="text-danger-600">*</span>
                </label>
                <Field
                  name="titre"
              ...
              <div>
                ...
                <Field
                  ...
                  name="description"
                  rows={3}
                  placeholder="Decrivez les missions..."
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Domaine <span className="text-danger-600">*</span>
                  </label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Localisation <span className="text-danger-600">*</span>
                  </label>
                  <Field
                    name="localisation"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Duree (mois) <span className="text-danger-600">*</span>
                  </label>
                  <Field
                    name="dureeMois"
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Date debut <span className="text-danger-600">*</span>
                  </label>
                  <Field
                    name="dateDebut"
                    type="date"
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      ...
                      if (e.target.value && values.dureeMois) {
                        ...
                        debut.setMonth(
                          debut.getMonth() + Number(values.dureeMois),
                        );
                        setFieldValue(
                          "dateExpiration",
                          debut.toISOString().split("T")[0],
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Date expiration <span className="text-danger-600">*</span>
                  </label>
                  <Field
                    name="dateExpiration"
                    type="date"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    Laissez vide si non remunere
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Nombre de postes <span className="text-danger-600">*</span>
                  </label>
                  <Field
                    name="nombrePostes"
                    type="number"
              ...
              <div>
                ...
                <Field
                  ...
                  rows={2}
                  placeholder="Ex: Java, Spring Boot, Angular (separees par des virgules)"
                  className="input-field resize-none"
                />
              </div>
